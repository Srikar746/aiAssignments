...
class State:
    ...
    def evaluate(self):
        ...
        if white_king_sq:
            attackers_white = self.board.attackers(chess.BLACK, white_king_sq)
            score -= 0.5 * len(attackers_white)

        if black_king_sq:
            attackers_black = self.board.attackers(chess.WHITE, black_king_sq)
            score += 0.5 * len(attackers_black)
...
def play_game():
    ...
    maxDepth = 3  # change depth to make AI stronger/weaker
    ...
    while not current_state.isTerminal():
        ...
        if current_state.player:  # Human = White
            try:
